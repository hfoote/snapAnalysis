...
class snapshot:
    ...
    def __init__(self, filename: str, ptype: int) -> None:
        ...
        with h5py.File(self.filename, "r") as f:
            ...
            self.length_unit = (f["Parameters"].attrs["UnitLength_in_cm"] * u.cm).to(
                u.kpc
            )
    ...
    def print_header(self):
        ...
        with h5py.File(self.filename, "r") as f:
            ...
            for (
                atr
            ) in attList:  # print all attributes in the header, with their contents
    ...
    def read_field(self, field: str) -> np.ndarray:
        ...
        if self.subset:
            raise RuntimeError(
                "Cannot read new fields after a subset of particles has been selected!"
            )
    ...
    def load_particle_data(self, fields: list) -> None:
        ...
        for field in fields:
            ...
            if ((field == "PsiRe") or (field == "PsiIm")) and not (
                self.fdm and self.ptype == 1
            ):
                raise RuntimeError(
            ...
            if field == "Masses":
                if (self.ptype == 1) and self.fdm:
                    self.data_fields[field] = (
                        np.abs(
                            self.read_field("PsiRe") ** 2
                            + self.read_field("PsiIm") ** 2
                        )
                        * self.mass_unit
                        / self.length_unit**3
                    )
                    ...
                    try:
                        self.data_fields[field] = (
                            self.read_field(field) * self.mass_unit
                        )
                    except KeyError:
                        self.data_fields[field] = (
    ...
    def get_FDM_velocities(self) -> None:
        ...
        vx[vx > np.pi] = vx[vx > np.pi] - 2.0 * np.pi
        vx[vx <= -np.pi] = vx[vx <= -np.pi] + 2.0 * np.pi
        vy[vy > np.pi] = vy[vy > np.pi] - 2.0 * np.pi
        vy[vy <= -np.pi] = vy[vy <= -np.pi] + 2.0 * np.pi
    ...
    def find_position_center(
        self,
        guess: None | np.ndarray = None,
        r_start: None | float = None,
        vol_dec: float = 3.0,
        delta: float = 0.01,
        N_min: int = 1000,
        verbose: bool = False,
        ...
        if guess is None:
            com = utils.com_define(
                self.data_fields["Masses"], self.data_fields["Coordinates"]
            )
        ...
        while change > delta:
            ...
            if len(m2) < N_min:
                warnings.warn(
                    "COM: Minimum number of particles reached before COM converged."
                )
                return com2
    ...
    def find_velocity_center(
        self, pos_center: np.ndarray, r_max: u.Quantity = 15.0 * u.kpc
    ) -> np.ndarray:
    ...
    def apply_rotation(self, rot_mat: np.ndarray) -> None:
        ...
        self.data_fields["Coordinates"] = (
            np.matmul(rot_mat, self.data_fields["Coordinates"].T)
        ).T

        if not self.check_if_field_read("Velocities"):
            warnings.warn("Velocities not loaded, rotation applied to positions only!")

        self.data_fields["Velocities"] = (
            np.matmul(rot_mat, self.data_fields["Velocities"].T)
        ).T
    ...
    def density_points(self, points: np.ndarray, k_max: int = 1000) -> np.ndarray:
        ...
        v = (
            ...
            / 3.0
            * np.pi
            * (dist.flatten() * self.field_units["Coordinates"]) ** 3.0
        )
